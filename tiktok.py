...
pygame.display.set_caption("Arc Game avec Score")
...
def compute_arc_points(r, t0, t1, n):
    return [(r*math.cos(t0+(t1-t0)*i/n), r*math.sin(t0+(t1-t0)*i/n)) for i in range(n+1)]
...
def spawn_arc(radius, ctype):
    ...
    def on_begin(arb,space_ref,_):
        if arc['active']:
            arc['active'] = False
            ball_shape = arb.shapes[0] if arb.shapes[0].collision_type==0 else arb.shapes[1]
            for bt in balls:
                if bt.shape == ball_shape:
                    bt.score += 1
                    break
...
for _ in range(5): spawn_arc(next_radius,next_ctype)

for dx,color in [(-50,(255,0,0)),(50,(0,0,255))]:
    b = pymunk.Body(1,pymunk.moment_for_circle(1,0,10))
    b.position = cx+dx,HEIGHT/2
    sh = pymunk.Circle(b,10)
    sh.elasticity=0.99; sh.friction=0.01; sh.collision_type=0
    space.add(b,sh)
    balls.append(BallTrail(b,sh,color))
...
while running:
    ...
    space.step(1/FPS)
    for bt in balls: bt.update()

    screen.fill((0,0,0))
    for arc in arcs:
        bdy=arc['body']
        for seg in arc['segments']:
            a=bdy.local_to_world(seg.a); c=bdy.local_to_world(seg.b)
            pygame.draw.line(screen,(255,255,255),a,c,ARC_THICKNESS)

    font = pygame.font.SysFont(None, 36)
    for i, bt in enumerate(balls):
        label = font.render(f"Score {i+1}: {bt.score}", True, bt.color)
        screen.blit(label, (10, 10 + i*30))

    for bt in balls: bt.draw(screen)
